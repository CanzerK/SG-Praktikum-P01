...
extension Device {
	...
	@objc func sendDriveWithHeadingCommand(speed: UInt8, heading: UInt16, direction: Direction, duration: Float, driveId: Int) {
		driveWithHeading(speed: 0, heading: heading, direction: direction)
		enqueueDelayCommand(0.1)
		driveWithHeading(speed: speed, heading: heading, direction: direction)
		enqueueDelayCommand(duration)
		driveWithHeading(speed: 0, heading: heading, direction: .forward) { [weak self] completion in
			guard let self = self else {
				return
			}
